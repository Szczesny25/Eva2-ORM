...
class JwtService
{
    ...
    public static function generar(array $payload, int $minutosExpiracion = 60): string
    {
        ...
        // Se agregan fechas de emisión y expiración al payload
        $payload['iat'] = time();
    ...
    public static function validar(string $token): ?array
    {
        ...
        // Si la firma no coincide, el token fue alterado o no es válido
        ...
        // Si ya pasó la fecha de expiración, el token no es válido
        if ($payload['exp'] < time()) {
            return null;
        }
